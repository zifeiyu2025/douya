...
// 50s 兼容二级降级链（Tavily 30s + Ollama 20s = 50s），
// 步进式降级无并发，单端超时累加需留足余量，否则下游 provider 永远拿不到机会。
// 防回归：避免被误改回 20s（20s 无法覆盖降级链，导致 Ollama 永远不执行）。
func TestDefaultSearchTimeout(t *testing.T) {
	got := DefaultSearchTimeout
	want := 50 * time.Second
	if got != want {
		t.Errorf("DefaultSearchTimeout = %v, 期望 %v（兼容二级降级链 Tavily+Ollama）", got, want)
	}
}

// TestTimeoutChainCompatibility 验证降级链超时累加不超过全局超时。
// Tavily 30s + Ollama 20s = 50s，应 <= DefaultSearchTimeout。
// 这是保证步进式降级链能完整执行的关键约束。
func TestTimeoutChainCompatibility(t *testing.T) {
	tavilyTimeout := 30 * time.Second
	// Ollama 的超时由 Provider 内部控制（20s），此处用估值验证累加上限
	ollamaEstimate := 20 * time.Second
	total := tavilyTimeout + ollamaEstimate
