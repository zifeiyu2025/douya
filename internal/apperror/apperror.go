...
// # 使用约定（对应审计报告 §4.2.3）
//
//   - 面向用户的业务/编排层（chat / config / app 层）：统一使用 Wrap / Wrapf，
//     传入中文消息与 ErrorKind，错误链经内部 %w 保留，调用方可用 errors.Is 判定；
//   - 底层库/协议包（tts、store 等需要将根因透传给上层判定的场景）：
//     允许直接 fmt.Errorf("...: %w", err)，但新代码优先 Wrap(KindXxx, ...)；
//   - 新增代码禁止裸 fmt.Errorf("... %w") 落在面向用户的错误路径上
//     （即上层会直接展示或据此分支的路径）。
package apperror
