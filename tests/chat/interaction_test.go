...
func TestSendMessage_ModelAutonomousThinking(t *testing.T) {
	...
	assistantMsgs, _ := store.GetMessagesByConversation(chat.GetDB(svc), convs[0].ID, nil)
...
func TestSendMessage_SearchEnabledFalse_ProvidesSearchTool(t *testing.T) {
	...
	if len(receivedReq.Tools) != 0 {
		t.Fatalf("expected 0 tool when SearchEnabled=false, got %d", len(receivedReq.Tools))
	}
}
...
//nolint:revive,staticcheck // 有意使用 SKIP_ 前缀标记跳过测试
...
func TestSendMessage_MultiRoundConversation(t *testing.T) {
	...
	msgs, _ := store.GetMessagesByConversation(chat.GetDB(svc), convID, nil)
	userCount := 0
	assistantCount := 0
	for _, m := range msgs {
		if m.Role == "user" {
...
func TestSendMessage_ToolDefinitionHasRichDescription(t *testing.T) {
	...
	if !strings.Contains(desc, "搜索") && !strings.Contains(desc, "search") {
		t.Errorf("tool description should mention search functionality, got: %s", desc)
	}
}
