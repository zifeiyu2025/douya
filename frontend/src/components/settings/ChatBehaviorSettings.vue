<!--
  ChatBehaviorSettings: 对话行为设置组件
  生活类比：像调教AI的性格——
    - 系统提示词 = 给AI的人设和规则
    - 温度 = AI回答的随机程度
    - TopP/TopK = 控制AI选词的确定性
    - 上下文长度 = AI能记住多长的对话历史
    - 重复惩罚 = 防止AI反复说同样的话

  设计要点：
    - 核心参数放在最前面（温度/TopP/TopK/上下文长度）
    - 高级参数折叠在"高级采样"子区
    - 模型参考参数卡片放在最前面，新手可一键应用官方推荐
-->
<template>
  <!-- 系统提示词 -->
  <n-form-item>
...
defineOptions({ name: 'ChatBehaviorSettings' })

// 空值保护：如果组件被误用在 SettingsView 之外，给出明确错误而非 undefined 崩溃
const ctx = inject<SettingsContext>(SETTINGS_CONTEXT_KEY)
if (!ctx) {
  throw new Error('ChatBehaviorSettings 必须在 SettingsView 内使用（缺少 settingsContext provide）')
}
const {
  formConfig,
  autoSave,
