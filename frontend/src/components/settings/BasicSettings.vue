<template>
  ...
  <n-form-item label="聊天背景">
    <div class="upload-wrapper">
      ...
      <div v-else class="upload-preview" @click="selectBackgroundImage">
  ...
  <n-form-item>
    <template #label>
      系统提示词
      <HelpTip
        content="追加在豆芽默认提示词后面的自定义指令，用于补充角色设定和行为约束。留空则仅使用默认提示词"
      />
    </template>
    <n-input
  ...
  <n-form-item>
    <template #label>
      推理模式
      <HelpTip
        content="控制模型的推理（思考）行为。on=始终开启推理，off=关闭推理，auto=由模型自行决定是否推理"
      />
    </template>
    <n-select
  ...
  <n-form-item>
    <template #label>
      推理预算
      <HelpTip
        content="限制推理（思考）过程的 token 数量。-1=无限（由模型自行决定），0=立即结束推理，N>0=限制为 N 个 token"
      />
    </template>
    <n-input-number
  ...
  <n-text
    v-if="formConfig.backend_sampling && supportsReasoning"
    depth="3"
    style="font-size: 12px; margin-top: -12px; display: block; margin-bottom: 8px"
  >
  ...
  <n-form-item>
    <template #label>
      温度
      <HelpTip content="控制回答的随机性。值越低越确定保守，值越高越多样创意。一般 0.3-0.8 之间" />
    </template>
  ...
  <n-form-item>
    <template #label>
      Top P
      <HelpTip
        content="从概率最高的候选词中筛选，只考虑累计概率达到此阈值的词。0.95 表示保留前 95% 概率的词"
      />
    </template>
  ...
  <n-form-item v-if="formConfig.dry_multiplier > 0">
    <template #label>
      DRY 允许长度
      <HelpTip content="允许重复的 token 序列长度。短于此长度的重复不会被惩罚，值越小越严格" />
    </template>
  ...
  <n-form-item>
    <template #label>
      自定义采样器顺序
      <HelpTip
        content="手动指定采样器的执行顺序，多个采样器用逗号分隔，如 top_k,top_p,temperature。留空则使用后端默认顺序"
      />
    </template>
    <n-input
  ...
  <n-form-item>
    <template #label>
      自适应采样衰减
      <HelpTip
        content="请求级自适应采样的衰减系数（0-1）。0=禁用，控制自适应调整的收敛速度，值越小调整越平缓"
      />
    </template>
    <n-input-number
  ...
  <div class="gen-params-save-row">
    <span v-if="genParamsDirty" class="gen-params-status">设置已修改，自动保存中...</span>
    <span v-else-if="formConfig.context_size > 0" class="gen-params-status saved">✓ 已保存</span>
  </div>
</template>

<script setup lang="ts">
import { inject } from 'vue'
import { NInputNumber, NSelect, NCheckbox } from 'naive-ui'
