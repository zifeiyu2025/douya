<template>
  ...
  <n-form-item>
    <template #label>
      本机服务地址
      <HelpTip
        content="llama-server 的 HTTP 服务地址。默认 http://127.0.0.1:8080，修改后需重启应用生效"
      />
    </template>
    <n-input
  ...
  <n-form-item label="暴露服务器地址" label-width="140" label-placement="left">
    <n-switch
      v-model:value="formConfig.expose_server"
      class="expose-switch"
      @update:value="onExposeServerToggle"
    />
    <span class="setting-hint">
      开启后局域网设备可通过本机 IP 访问（需重启服务生效）。开启后必须配置 API
      Key，否则服务将拒绝启动
    </span>
  </n-form-item>
  ...
  <n-form-item label-width="140" label-placement="left">
    <template #label>
      启用原生 Web UI
      <HelpTip
        content="开启后可通过浏览器访问 llama-server 自带的 Web UI（地址为本机服务地址）。与豆芽前端独立，仅供高级用户调试。需重启服务生效"
      />
    </template>
  ...
  <n-form-item label-width="140" label-placement="left">
    <template #label>
      启用 API Key 验证
      <HelpTip
        content="开启后所有 API 请求需携带 API Key，防止未授权访问。暴露到局域网时强烈建议开启"
      />
    </template>
    <n-switch
  ...
  <n-form-item>
    ...
    <n-select
      ...
      placeholder="自动检测"
      clearable
      :disabled="formConfig.spec_default"
    />
  </n-form-item>
  <n-text
  ...
  <n-form-item
    v-if="
      formConfig.spec_type === 'draft-mtp' ||
      formConfig.spec_type === 'draft-eagle3' ||
      formConfig.spec_type === 'draft-dflash' ||
      formConfig.spec_type === 'draft-simple'
    "
