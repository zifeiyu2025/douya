<template>
  ...
  <n-form-item>
    ...
    <n-switch
      v-model:value="formConfig.agent"
      :disabled="formConfig.ui_mcp_proxy"
  ...
  <n-form-item>
    ...
    <n-switch
      v-model:value="formConfig.ui_mcp_proxy"
  ...
  <n-form-item>
    <template #label>
      虚拟滚动
      <HelpTip
        content="实验性功能。开启后长会话（100+ 条消息）仅渲染可见区域内的消息，降低内存占用与滚动开销；可能存在滚动跳动或消息重新进入视口时短暂重渲染。关闭即可恢复默认渲染"
      />
    </template>
...
const { formConfig, autoSave, handleAgentChange, handleBackendSamplingChange } = ctx
