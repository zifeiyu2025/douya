...
export function useMorphRender() {
  ...
  function doRenderSync(content: string) {
    ...
    for (let i = 0; i < stableCount; i++) {
      ...
      if (!html) {
        // LRU 淘汰：超限时删除最旧的 key（Map 保留插入顺序，keys().next().value 即最旧）
        ...
        // cache hit 时 delete + set 把命中项移到 Map 末尾，实现真正的 LRU
        // （只读不更新位置的话会退化为 FIFO：频繁复用的早期 block 仍可能被当作"最旧"淘汰）
